...
export class SearchService {
  ...
  async search(query: string): Promise<any[]> {
    ...
    // 3. Vector Similarity Search via pgvector raw query (if ENABLE_SEMANTIC_SEARCH=true)
    const enableSemantic = this.configService.get<string>('ENABLE_SEMANTIC_SEARCH') === 'true' && this.embeddingService.isConfigured;

    if (enableSemantic) {
      try {
        const { vector } = await this.embeddingService.embed(cleanQuery);
        const vectorStr = `[${vector.join(',')}]`;

        // Query our new SearchEmbedding table with join
        const vectorMatches: any[] = await prisma.$queryRawUnsafe(
          `SELECT idx."datasetId", idx."contentType", (1 - (se.vector <=> $1::vector)) as similarity
           FROM "SearchEmbedding" se
           JOIN "SearchIndex" idx ON se."searchIndexId" = idx."id"
           ORDER BY se.vector <=> $1::vector ASC
           LIMIT 20`,
          vectorStr
        );
